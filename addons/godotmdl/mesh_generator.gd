...
static func generate_mesh(mdl: MDLReader, vtx: VTXReader, vvd: VVDReader, phy: PHYReader, options: Dictionary) -> MeshInstance3D:
	...
	_generate_collision(mesh_instance, skeleton, phy);
	_assign_materials(array_mesh, mdl, mesh_instance);

	return mesh_instance;

static func _generate_collision(root: Node3D, skeleton: Skeleton3D, phy: PHYReader):
	...
	for surface in phy.surfaces:
		...
		for solid in surface.solids:
			...
			for face in solid.faces:
				var v1 = surface.vertices[face.v1];
				var v2 = surface.vertices[face.v2];
				var v3 = surface.vertices[face.v3];
...
static func _assign_materials(mesh: ArrayMesh, mdl: MDLReader, mesh_instance):
	var materials = [];
	var skin = 0;

	for tex in mdl.textures:
		for dir in mdl.textureDirs:
			var path = ("res://materials/" + dir + "/" + tex.name + ".vmt").to_lower();

			if not FileAccess.file_exists(path): continue;

			materials.append(VMTImporter.load(path));
			break;
